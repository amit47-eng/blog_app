...
const communicationSchema = new mongoose.Schema<ICommunication>({
  ...
  country: { type: String, enum: ["INDIA", "US", "AUSTRALIA"], default: "INDIA" },
...
const userSchema = new mongoose.Schema<IUser>(
  {
    firstname: { type: String, required: [true, "Please enter the First Name"], minLength: 3 },
    lastname: { type: String, required: [true, "Please enter the Second Name"], minLength: 3 },
    about: { type: String, required: [true, "Please enter the Summary of the user"], minLength: 10, maxLength: 200 },
    email: {
      type: String,
      lowercase: true,
    ...
    password: { type: String, required: [true, "Please input the user Password"] },
    username: { type: String, required: [true, "Please input the username"], unique: true, minLength: 3 },
    role: { type: String, enum: ["ADMIN", "USER", "QA-TESTER", "CONTENT_WRITTER"], default: "USER" },
    ...
    phoneNumber: {
      ...
      type: String,
      minLength: 10,
      match: [/^\(?\+?[0-9]*\)?[-.\s]?[0-9]+[-.\s]?[0-9]+$/, "Please provide a valid phone number"],
    },
